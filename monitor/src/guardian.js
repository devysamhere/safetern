...
export class TelegramGuardian {
  ...
  mergeLocalState(next = {}) {
    const current = this.readLocalState();
    ...
    const merged = {
      ...
      update_offset: Math.max(
        Number(current.update_offset || 0),
        Number(next.update_offset || 0)
      ),

      notified_event_ids: [
        ...new Set([
          ...(current.notified_event_ids || []),
          ...(next.notified_event_ids || []),
        ]),
      ].slice(-500),
    };

    this.writeLocalState(merged);
    return merged;
  }
  ...
  readConnections() {
    try {
      return JSON.parse(
        fs.readFileSync(this.connectionsPath, "utf8")
      );
    } catch {
      return {
        wallets: {},
  ...
  createSignatureChallenge(wallet, action = "connect") {
    const state = this.pruneConnections(
  ...
  completePair(token, chatId, user = {}) {
    ...
    if (
      previous?.chat_id &&
      state.chats?.[String(previous.chat_id)]?.wallet ===
        wallet
    ) {
    ...
    const priorWallet =
      state.chats?.[String(chatId)]?.wallet;

    if (
      priorWallet &&
      state.wallets?.[priorWallet]?.chat_id ===
        String(chatId)
    ) {
  ...
  async call(method, payload = {}) {
    ...
    if (!response.ok || data?.ok === false) {
      throw new Error(
        data?.description || `Telegram ${response.status}`
      );
    }
  ...
  async send(text, extra = {}) {
    return this.chatId
      ? this.sendTo(this.chatId, text, extra)
      : false;
  }
  ...
  async registerCommands() {
    ...
    try {
      ...
      console.log(
        "Safetern Guardian Telegram commands registered."
      );
    } catch (error) {
      console.error(
  ...
  homeMessage(wallet = "") {
    ...
    if (wallet) {
      lines.push(
        "",
        `Connected wallet: ${shortWallet(wallet)}`
      );
    }
  ...
  audienceForRecord(record = {}) {
    ...
    if (
      ...
      wallets.add(
        norm(record.recovery_controller)
      );
    }
    ...
    for (const wallet of wallets) {
      const chat =
        connections.wallets?.[wallet]?.chat_id;

      if (chat) {
        chats.add(String(chat));
      }
    }

    if (this.chatId) {
      chats.add(this.chatId);
    }
  ...
  async sendChallengeTo(chatId, event) {
    ...
    const left = Math.max(
      0,
      Number(event.challenge_expires_at || 0) -
        now
    );
    ...
    const lines = [
      ...
      "",
      event.name ||
        `Covenant #${event.record_id}`,
      `Mode: ${event.mode || "—"}`,
      `GenLayer result: ${
        event.classification || "ABANDONED"
      }${
        event.confidence
          ? ` · ${event.confidence}%`
          : ""
      }`,
      `Challenge window remaining: ${duration(
        left
      )}`,
    ];
  ...
  messageForEvent(event, state) {
    ...
    if (
      ...
      if (event.dex_liquidity_usd) {
        lines.push(
          `DEX liquidity: ${shortMoney(
            event.dex_liquidity_usd
          )}`
        );
      }
    ...
    if (
      ...
      if (event.mode === "RECOVER") {
        ...
        event.recovery_controller
      ) {
        lines.push(
          `Recovery controller: ${shortWallet(
            event.recovery_controller
          )}`
        );
      }
    ...
    if (
      event.type ===
        "assessment_pending_timeout" ||
      event.type ===
        "recovery_finalize_pending_timeout"
    ) {
      return [
        `Safetern Guardian · ${
          event.type ===
      ...
      for (const chatId of chats) {
        try {
          if (
            ...
            await this.sendChallengeTo(
              chatId,
              event
            );
          } else {
            const message =
              this.messageForEvent(
                event,
                state
              );
  ...
  formatStatus(state, wallet = "") {
    ...
    for (
      const record of records.slice(0, 16)
    ) {
      const classification =
        record.latest_classification ||
        "NOT ASSESSED";
      ...
      lines.push(
        `Last assessment: ${classification}${confidence}`
      );
      ...
      if (
        record.assessment_pending
      ) {
        lines.push(
          "GenLayer assessment: pending"
        );
      }
  ...
  formatRecord(
    ...
    lines.push(
      `Assessment cadence: ${Math.max(
        ...
        Math.round(
          Number(
            record.onchain_assessment_interval_seconds ||
              0
          ) / 3600
        )
      )}h`
