...
const DEMO_ENABLED = String(process.env.DEMO_ENABLED || "false").toLowerCase() === "true";
const DEMO_POOL = Array.from({ length: 5 }, (_, index) => {
  const n = index + 1;
  return {
    ownerKey: String(process.env[`DEMO_POOL_${n}_OWNER_PRIVATE_KEY`] || "").trim(),
    beneficiaryKey: String(process.env[`DEMO_POOL_${n}_BENEFICIARY_PRIVATE_KEY`] || "").trim(),
  };
});
...
const guardian = new TelegramGuardian({
  root: ROOT,
  token: TELEGRAM_BOT_TOKEN,
  chatId: TELEGRAM_CHAT_ID,
  contract: CONTRACT,
  appUrl: SAFETERN_APP_URL,
  botUsername: TELEGRAM_BOT_USERNAME
});
const demoWallets = new SafeternDemoWallets({
...
function readState() {
  try {
    ...
    return { watches: {}, records: {}, events: [], updated_at: 0 };
  }
}
...
async function getText(url) {
  const response = await fetch(url, {
    headers: {
      "user-agent": "Safetern-Monitor/0.1",
...
async function fetchDex(metadata) {
  ...
  if (!chain || !address) {
    return { available: false, error: "Missing chain or contract address" };
  }
  ...
  try {
    ...
    for (const pair of pairs) {
      ...
      if (Number.isFinite(liq) && liq > 0) {
        total += liq;
        pools += 1;
        largest = Math.max(largest, liq);
      }
      const vol = Number(pair?.volume?.h24 || 0);
      if (Number.isFinite(vol) && vol > 0) volume24h += vol;
    }
    ...
    return {
      ...
      url,
      chain,
      total_liquidity_usd: Math.round(total),
      largest_pool_usd: Math.round(largest),
      pool_count: pools,
...
async function fetchExchangeSignals(metadata) {
  ...
  const selected =
    Array.isArray(metadata.major_exchanges) && metadata.major_exchanges.length
      ? metadata.major_exchanges
  ...
  for (const name of selected) {
    if (ANNOUNCEMENT_URLS[name]) {
      try {
        ...
        result[name] = {
          source: ANNOUNCEMENT_URLS[name],
          delisting_mention: symbolMentionedNearDelist(body, symbol),
          status: "checked"
        };
      } catch (error) {
        result[name] = {
          source: ANNOUNCEMENT_URLS[name],
          delisting_mention: false,
          status: "error",
          error: error?.message || String(error)
        };
      }
      ...
      try {
        const products = JSON.parse(
          await getText("https://api.exchange.coinbase.com/products")
        );
        ...
        result[name] = {
          source: "https://api.exchange.coinbase.com/products",
          listed: matches.length > 0,
          matching_spot_products: matches.slice(0, 10).map(p => p.id),
          status: "checked"
        };
      } catch (error) {
        result[name] = {
...
async function readContinuityRows() {
  if (!CONTRACT || !CONTRACT.startsWith("0x")) {
    throw new Error("Set SAFETERN_CONTRACT in monitor/.env.");
  }

  const count = Number(
    await readContractThrottled({
      address: CONTRACT,
  ...
  for (let id = 1; id <= count; id++) {
    const record = await readContractThrottled({
      address: CONTRACT,
    ...
    const monitoring = await readContractThrottled({
      address: CONTRACT,
      functionName: "get_monitoring_config",
      args: [id],
      stateStatus: "accepted"
    });
    ...
    let latest = null;
    try {
      latest = await readContractThrottled({
        address: CONTRACT,
    ...
    if (record?.mode === "WATCH") {
      try {
        const wm = await readContractThrottled({
          address: CONTRACT,
          functionName: "get_watch_metadata",
          args: [id],
          stateStatus: "accepted"
        });
...
async function maybeAssess(row, reasons, state) {
  ...
  try {
    ...
    addEvent(state, {
      type: "assessment_trigger_failed",
      record_id: row.id,
      name: row.record.name,
      reasons,
      error: error?.message || String(error)
...
async function maybeFinalizeRecovery(row, state) {
  ...
  try {
    ...
    addEvent(state, {
      type: "recovery_finalize_triggered",
      record_id: row.id,
      name: row.record.name,
      mode: row.record.mode,
      beneficiary: row.record.beneficiary || "",
      tx_hash: hash
    });
...
async function pollOnce() {
  ...
  for (const row of rows) {
    ...
    const latestClassification = row.latest?.found
      ? String(row.latest.classification || "")
      : "";
    const latestConfidence = row.latest?.found
      ? Number(row.latest.confidence || 0)
      : 0;
    const latestAssessmentId = row.latest?.found
      ? Number(row.latest.assessment_id || 0)
      : 0;
    ...
    let dex = null;
    let exchanges = null;
    let liquidityChange = null;
    let confirmedMentions = [];
    let newConfirmedMentions = [];

    if (mode === "WATCH" && metadata.watch_type === "CRYPTO_TOKEN") {
      if (metadata.track_dex_liquidity) dex = await fetchDex(metadata);
      if (metadata.track_exchange_delistings) {
        exchanges = await fetchExchangeSignals(metadata);
      }

      const oldLiquidity = Number(prior?.dex?.total_liquidity_usd || 0);
      const newLiquidity = Number(dex?.total_liquidity_usd || 0);

      if (oldLiquidity > 0 && newLiquidity >= 0) {
        liquidityChange =
          ((newLiquidity - oldLiquidity) / oldLiquidity) * 100;
      }

      const currentMentions = Object.entries(exchanges || {})
        .filter(([, v]) => v?.delisting_mention)
        .map(([name]) => name);

      const priorMentions = Object.entries(prior?.exchanges || {})
        .filter(([, v]) => v?.delisting_mention)
        .map(([name]) => name);

      const priorConfirmedMentions =
        Array.isArray(prior?.confirmed_delisting_exchanges)
          ? prior.confirmed_delisting_exchanges
          : [];

      confirmedMentions = currentMentions.filter(name =>
        priorMentions.includes(name)
      );

      newConfirmedMentions = confirmedMentions.filter(
        name => !priorConfirmedMentions.includes(name)
      );

      if (
        liquidityChange !== null &&
        liquidityChange <= -ALERT_PCT
      ) {
        addEvent(state, {
          type: "liquidity_alert",
          record_id: row.id,
          name: record.name,
          change_percent: Number(liquidityChange.toFixed(2)),
          previous_usd: oldLiquidity,
          current_usd: newLiquidity
        });
      }

      if (newConfirmedMentions.length) {
        addEvent(state, {
          type: "delisting_alert",
          record_id: row.id,
          name: record.name,
          exchanges: newConfirmedMentions,
    ...
    if (
      ...
      latestClassification !== priorClassification
    ) {
      let market = {};

      if (mode === "WATCH") {
        try {
          const snap = await readContractThrottled({
            address: CONTRACT,
            functionName: "get_watch_market_snapshot",
            args: [Number(row.id)],
            stateStatus: "accepted"
          });
      ...
      addEvent(state, {
        ...
        name: record.name,
        mode,
        previous_classification: priorClassification,
        classification: latestClassification,
        confidence: latestConfidence,
        evidence_summary: row.latest?.evidence_summary || "",
        dex_liquidity_usd: Number(
          market?.dex?.total_liquidity_usd ||
            market?.total_liquidity_usd ||
            0
        ),
        liquidity_change_percent:
          market?.liquidity_change_percent ??
    ...
    if (pendingAssessment) {
      const baseline = Number(
        pendingAssessment.baseline_last_assessment_at || 0
      );
      ...
      if (lastAssessment > baseline) {
        addEvent(state, {
          ...
          name: record.name,
          mode,
          tx_hash: pendingAssessment.tx_hash || "",
          submitted_at: pendingAssessment.submitted_at || 0,
          onchain_last_assessment_at: lastAssessment
        });
        pendingAssessment = null;
      } else if (
        now >=
        Number(pendingAssessment.submitted_at || 0) +
          PENDING_RETRY_SECONDS
      ) {
        addEvent(state, {
          type: "assessment_pending_timeout",
          record_id: row.id,
          name: record.name,
          mode,
          tx_hash: pendingAssessment.tx_hash || "",
    ...
    if (pendingFinalize) {
      if (currentState === "RECOVERED") {
        ...
        now >=
        Number(pendingFinalize.submitted_at || 0) +
          FINALIZE_PENDING_RETRY_SECONDS
      ) {
        addEvent(state, {
          type: "recovery_finalize_pending_timeout",
          record_id: row.id,
          name: record.name,
          mode,
          tx_hash: pendingFinalize.tx_hash || "",
          submitted_at: pendingFinalize.submitted_at || 0
        });
        pendingFinalize = null;
      }
    }
    ...
    if (mode === "WATCH") {
      if (
        liquidityChange !== null &&
        liquidityChange <= -SEVERE_PCT
      ) {
        reasons.push(
          `severe_liquidity_drop_${Math.abs(
            liquidityChange
          ).toFixed(1)}pct`
        );
      } else if (
      ...
      if (newConfirmedMentions.length) {
        reasons.push(
          `confirmed_delisting_signal_${newConfirmedMentions.join("_")}`
        );
      }
    }

    if (scheduledDue) {
      reasons.push("scheduled_assessment_due");
    }
    ...
    const common = {
      ...
      mode,
      name: record.name,
      entity: record.protected_entity,
      owner: record.owner || "",
      beneficiary: record.beneficiary || "",
      recovery_controller: record.recovery_controller || "",
      state: currentState,
      ...
      recovered_at: Number(record.recovered_at || 0),
      checked_at: now,
      next_lightweight_check_at: now + POLL_SECONDS,
      assessment_due: reasons.length > 0,
      assessment_reasons: reasons,
      onchain_last_assessment_at: lastAssessment,
      onchain_assessment_interval_seconds: interval,
    ...
    if (mode === "WATCH") {
      state.watches[key] = {
        ...common,
        watch_type: metadata.watch_type,
        token_symbol: metadata.token_symbol || "",
        blockchain: metadata.blockchain || "",
        contract_address: metadata.contract_address || "",
        dex,
        liquidity_change_since_last_poll_percent:
          liquidityChange === null
            ? null
            : Number(liquidityChange.toFixed(2)),
        exchanges,
    ...
    if (!pendingAssessment && reasons.length) {
      ...
      if (hash) {
        const p = {
          tx_hash: hash,
          submitted_at: now,
          baseline_last_assessment_at: lastAssessment,
          reasons: [...reasons]
        };
        ...
        if (mode === "WATCH") {
          state.watches[key].assessment_pending = true;
          state.watches[key].pending_assessment = p;
        }
      }
    }
    ...
    const challengeExpired =
      mode !== "WATCH" &&
      currentState === "CHALLENGE" &&
      Number(record.challenge_expires_at || 0) > 0 &&
      now >= Number(record.challenge_expires_at || 0);
  ...
  const activeWatches = Object.values(
    state.watches || {}
  ).filter(w => w?.active).length;

  const activeCovenants = Object.values(
    state.records || {}
  ).filter(r => r?.active && r?.mode !== "WATCH").length;

  console.log(
    `[${new Date().toISOString()}] monitored ${activeWatches} Watch record(s), ${activeCovenants} covenant(s)`
  );
...
async function lifecyclePollOnce() {
  ...
  try {
    ...
    for (const prior of candidates) {
      ...
      try {
        record = await readContractThrottled({
          address: CONTRACT,
          functionName: "get_record",
          args: [id],
          stateStatus: "accepted"
      ...
      try {
        monitoring = await readContractThrottled({
          address: CONTRACT,
          functionName: "get_monitoring_config",
          args: [id],
          stateStatus: "accepted"
      ...
      const priorState = String(prior.state || "");
      const lastAssessment = Number(
        monitoring?.last_assessment_at ??
          prior.onchain_last_assessment_at ??
          0
      );
      const latestClassification = latest?.found
        ? String(latest.classification || "")
        : String(prior.latest_classification || "");
      const latestConfidence = latest?.found
        ? Number(latest.confidence || 0)
        : Number(prior.latest_confidence || 0);
      const latestAssessmentId = latest?.found
        ? Number(latest.assessment_id || 0)
        : Number(prior.latest_assessment_id || 0);

      let pendingAssessment =
        prior.pending_assessment || null;
      ...
      if (currentState && currentState !== priorState) {
        if (currentState === "CHALLENGE") {
          addEvent(state, {
            ...
            beneficiary:
              record.beneficiary || prior.beneficiary || "",
            challenge_expires_at: Number(
              record.challenge_expires_at || 0
            ),
            confidence: latestConfidence,
          ...
          addEvent(state, {
            ...
            beneficiary:
              record.beneficiary || prior.beneficiary || "",
            recovery_controller:
              record.recovery_controller ||
              prior.recovery_controller ||
              "",
            recovered_at: Number(record.recovered_at || 0)
          });
        }
      ...
      state.records[key] = {
        ...
        state: currentState,
        active: Boolean(record.active),
        challenge_started_at: Number(
          record.challenge_started_at || 0
        ),
        challenge_expires_at: Number(
          record.challenge_expires_at || 0
        ),
      ...
      if (challengeExpired && !pendingFinalize) {
        const hash = await maybeFinalizeRecovery(
          { id, record },
          state
        );
...
async function verifyGuardianAuthorization({
  wallet,
  nonce,
  signature,
  action
  ...
  if (!nonce || !signature) {
    throw new Error("Missing wallet authorization");
  }

  const challenge =
    guardian.consumeSignatureChallenge(
      nonce,
      wallet,
      action
    );
  ...
  return http
    .createServer(async (req, res) => {
      if (req.method === "OPTIONS") {
        res.writeHead(204, {
          ...
          "access-control-allow-headers":
            "content-type,x-safetern-demo-token"
        });
        return res.end();
      }
      ...
      try {
        if (url.pathname === "/health") {
          const pending_assessments =
            Object.values(state.records || {}).filter(
              r => r?.assessment_pending
            ).length;

          const pending_finalizations =
            Object.values(state.records || {}).filter(
              r => r?.finalize_pending
            ).length;

          return sendJson(res, 200, {
            ok: true,
            service: "Safetern Monitor",
            contract: CONTRACT,
            auto_assess:
              AUTO_ASSESS && Boolean(keeperClient),
            auto_finalize:
              AUTO_FINALIZE && Boolean(keeperClient),
            telegram_guardian: guardian.configured,
            guardian_pairing: guardian.configured,
            embedded_demo: demoWallets.enabled,
        ...
        /*
         * Create a NEW embedded demo session.
         */
        if (
          ...
          return sendJson(
            res,
            200,
            demoWallets.createSession(ip)
          );
        }

        /*
         * Restore/validate an EXISTING embedded demo session.
         *
         * Unlike POST /demo/session, this route never allocates
         * another wallet pair. The existing token must still be
         * valid and belong to this connection.
         */
        if (
          url.pathname === "/demo/session" &&
          req.method === "GET"
          ...
          const token = String(
            req.headers["x-safetern-demo-token"] ||
              ""
          );
        ...
        if (
          ...
          return sendJson(
            res,
            200,
            await demoWallets.signRecoveryMessage({
              token,
              ip,
              role: body.role,
        ...
        if (
          ...
          return sendJson(
            res,
            200,
            demoWallets.guardianDisconnect({
              token,
              ip,
              role: body.role
            })
        ...
        if (
          ...
          if (!guardian.configured) {
            return sendJson(res, 503, {
              error:
                "Telegram Guardian is not configured"
            });
          }
          ...
          return sendJson(
            res,
            200,
            guardian.createSignatureChallenge(
              wallet,
              action
            )
          );
        ...
        if (
          ...
          await verifyGuardianAuthorization({
            wallet: body.wallet,
            nonce: body.nonce,
          ...
          const pair =
            guardian.createPairToken(body.wallet);
    ...
    .listen(PORT, () =>
      console.log(
        `Safetern Monitor API: http://localhost:${PORT}`
      )
    );
}
...
if (once) {
  ...
  guardian.initializeExistingEvents(
    initialState.events || []
  );
  ...
  console.log(
    `Initial full monitor scan scheduled in ${INITIAL_POLL_DELAY_SECONDS}s (RPC reads throttled to one every ${RPC_READ_INTERVAL_MS}ms).`
  );

  setTimeout(
    () => pollOnce().catch(console.error),
    INITIAL_POLL_DELAY_SECONDS * 1000
  );
