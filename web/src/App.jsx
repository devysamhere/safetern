...
function shortAddress(value = "") { return value ? `${value.slice(0, 6)}â€¦${value.slice(-4)}` : ""; }
function formatUsd(value) { const n=Number(value||0); if(!Number.isFinite(n)) return "â€”"; return new Intl.NumberFormat("en-US",{style:"currency",currency:"USD",notation:n>=1000000?"compact":"standard",maximumFractionDigits:n>=1000000?2:0}).format(n); }
function formatPct(value) { const n=Number(value); if(!Number.isFinite(n)) return "â€”"; return `${n>0?"+":""}${n.toFixed(1)}%`; }
...
function Notice({ notice, onClear }) {
  ...
  return <div className={`notice ${notice.kind || ""}`}><div><span className="notice-dot"/><span>{notice.text || notice}</span></div><div className="notice-actions">{notice.hash && <button onClick={() => navigator.clipboard?.writeText(notice.hash)}>{shortAddress(notice.hash)} Â· Copy</button>}<button className="notice-close" onClick={onClear}>Ã—</button></div></div>;
}

function SourceFields({ form, setForm }) {
  return <div className="form-section"><div className="field-label"><span>Evidence sources</span><small>Public http/https URLs Â· max 5</small></div>{form.sources.map((value,index)=><div className="source-row" key={index}><input required={index===0} type="url" value={value} onChange={(e)=>{const sources=[...form.sources];sources[index]=e.target.value;setForm({...form,sources});}} placeholder={`Evidence URL ${index+1}`}/>{index>0&&<button type="button" onClick={()=>setForm({...form,sources:form.sources.filter((_,i)=>i!==index)})}>Remove</button>}</div>)}{form.sources.length<5&&<button type="button" className="add-source" onClick={()=>setForm({...form,sources:[...form.sources,""]})}>+ Add source</button>}</div>;
}
...
function App() {
  ...
  async function onConnect(){
    ...
    setBusy("connect"); setNotice({text:"Opening your walletâ€¦"});
    try {
  ...
  async function onDisconnect(){
    clearStoredDemoSession(); demoActiveRef.current=false; setDemoSession(null); delete window.__SAFETERN_DEMO_SIGN__;
    setBusy("disconnect");
    setNotice({text:"Disconnecting Safetern from this walletâ€¦"});
    try {
  ...
  function applyDemoRole(session, role) {
    ...
    refresh({force:true,address}).catch(()=>{});
  }
  async function startEmbeddedDemo(role=demoRole) {
    setBusy("demo-session"); setNotice({text:"Preparing disposable Safetern Studionet demo walletsâ€¦"});
    try {
  ...
  async function submitCreate(kind,form,createFn){setBusy(`create-${kind}`);beginTxStage();setNotice({text:`Preparing ${kind} transactionâ€¦`});try{const before=Number((await getRecordCount())||0);const result=await createFn(client,form);const hash=txHash(result);setTxStage("submitted");setNotice({text:"Transaction submitted. Waiting for GenLayer validator consensusâ€¦",hash});await waitForReceipt(hash);setTxStage("consensus");const appeared=await waitFor(async()=>Number((await getRecordCount())||0)>before);await refresh();const recordId=appeared?before+1:null;setCreationResult({kind,name:form.name||"Safetern record",hash,recordId,monitoringInterval:kind==="watch"?form.monitoringInterval:null,appeared});setNotice(null);return appeared;}catch(e){setNotice({text:actionErrorMessage(e,"Transaction failed."),kind:"error"});return false;}finally{setBusy("");setTxStage("");}}
  ...
  async function onCreateRecover(e){e.preventDefault();if(!client){setNotice({text:"Connect the owner wallet first.",kind:"error"});return;}setBusy("create-recover");setNotice({text:"Verifying beneficiary Recovery Key and encrypting locallyâ€¦"});try{await verifyRecipientCode(recoverForm.beneficiaryCode,recoverForm.beneficiary);const encrypted=await encryptRecoveryPayload({secret:recoverForm.secret,payloadType:recoverForm.payloadType,beneficiaryCode:recoverForm.beneficiaryCode,beneficiaryAddress:recoverForm.beneficiary});const payload={...recoverForm,encryptedPayloadRef:encrypted.ref,encryptedPayloadHash:encrypted.hash};const ok=await submitCreate("recover",payload,createRecovery);if(ok)setRecoverForm({name:"",description:"",entity:"Personal recovery information",beneficiary:"",beneficiaryCode:"",payloadType:"Recovery instructions",secret:"",rule:RECOVER_RULE,policy:"STRICT",challengeSeconds:"86400",sources:["",""]});}catch(e){setNotice({text:e?.message||"Could not create encrypted recovery covenant.",kind:"error"});}finally{setBusy("");}}

  async function onAssess(id){setBusy(`assess-${id}`);beginTxStage();setNotice({text:`Submitting assessment for record #${id}â€¦`});try{let previous=0;try{previous=Number((await getLatestAssessment(id))?.assessment_id||0);}catch{}const result=await assessRecord(client,id);const hash=txHash(result);setTxStage("submitted");setNotice({text:"Assessment submitted. Waiting for GenLayer validator consensusâ€¦",hash});await waitForReceipt(hash);setTxStage("consensus");const completed=await waitFor(async()=>{const latest=await getLatestAssessment(id);return latest?.found&&Number(latest.assessment_id)>previous?latest:null;},{timeoutMs:180000,intervalMs:4000});await refresh();setNotice({text:completed?"Assessment finalized by GenLayer.":"Assessment is still processing; refresh shortly.",hash,kind:completed?"success":""});}catch(e){setNotice({text:actionErrorMessage(e,"Assessment failed."),kind:"error"});}finally{setBusy("");setTxStage("");}}
  async function stateAction(action,id){setBusy(`${action}-${id}`);beginTxStage();setNotice({text:action==="presence"?"Confirming owner presenceâ€¦":"Finalizing recoveryâ€¦"});try{const fn=action==="presence"?confirmPresence:finalizeRecovery;const result=await fn(client,id);const hash=txHash(result);setTxStage("submitted");setNotice({text:"Transaction submitted. Waiting for GenLayer validator consensusâ€¦",hash});await waitForReceipt(hash);setTxStage("consensus");const target=action==="presence"?"HEALTHY":"RECOVERED";let observed=null;try{observed=await waitFor(async()=>((await getRecord(id))?.state===target),{timeoutMs:45000,intervalMs:5000});}catch{}try{await refresh({force:true});}catch{}setNotice({text:action==="presence"?(observed?"Presence confirmed. Recovery challenge cancelled.":"Presence transaction submitted. Safetern will refresh the onchain state when the RPC is available."):(observed?"Recovery finalized. Successor authorization is now active.":"Recovery finalization submitted. Safetern will refresh when the RPC is available."),hash,kind:"success"});}catch(e){setNotice({text:actionErrorMessage(e,"Action failed."),kind:"error"});}finally{setBusy("");setTxStage("");}}
  async function claimAccess(id){setBusy(`claim-${id}`);beginTxStage();setNotice({text:"Claiming beneficiary recovery accessâ€¦"});try{const result=await claimRecoveryAccess(client,id);const hash=txHash(result);setTxStage("submitted");setNotice({text:"Transaction submitted. Waiting for GenLayer validator consensusâ€¦",hash});await waitForReceipt(hash);setTxStage("consensus");await waitFor(async()=>{const a=await getRecoveryAccess(id);return a?.found?a:null;});await refresh();setNotice({text:"Beneficiary access claimed onchain. You can now unlock the encrypted recovery information.",hash,kind:"success"});}catch(e){setNotice({text:actionErrorMessage(e,"Could not claim recovery access."),kind:"error"});}finally{setBusy("");setTxStage("");}}
  ...
  async function createIdentity(){setBusy("identity");setNotice({text:"Creating Recovery Identity locallyâ€¦ One wallet signature will protect the private recovery key."});try{const made=await generateRecoveryIdentity(account);const code=recipientCodeFromIdentity(made);setIdentity(made);setRecipientCode(code);setNotice({text:"Recovery Identity created. Confirm one GenLayer transaction to register the public key to this wallet."});const result=await registerRecoveryIdentity(client,code,made.fingerprint);const hash=txHash(result);setNotice({text:"Registration submitted. Waiting for GenLayer validator consensusâ€¦",hash});await waitForReceipt(hash);const registered=await waitFor(async()=>{const r=await getRegisteredRecoveryIdentity(account);return r?.found?r:null;});setNotice({text:registered?"Recovery Identity registered and ready.":"Identity created locally; onchain registration is still finalizing.",hash,kind:registered?"success":""});}catch(e){setNotice({text:actionErrorMessage(e,"Could not create Recovery Identity."),kind:"error"});}finally{setBusy("");}}
  ...
  function renderCard(row){const {record,assessment,access,watchMetadata,monitoring,marketSnapshot}=row;const assessed=Boolean(assessment);const status=record.state==="CHALLENGE"||record.state==="RECOVERED"?record.state:(assessed?assessment.classification:"UNASSESSED");const [label,desc]=statusCopy[status]||[status,"Continuity state recorded onchain."];const sources=normalizeSources(record.evidence_sources_json);const secondsLeft=record.state==="CHALLENGE"?Math.max(0,Number(record.challenge_expires_at)-Math.floor(tick/1000)):0;const isOwner=sameAddress(account,record.owner);const isBeneficiary=sameAddress(account,record.beneficiary);return <article className="record-card" key={record.record_id}>
    ...
    {record.mode==="WATCH"&&<div className="meta-grid watch-meta-grid"><span>Watch type<b>{watchMetadata?.watch_type==="CRYPTO_TOKEN"?"Crypto token":"Project"}</b></span><span>Monitoring<b>{formatDuration(monitoring?.interval_seconds||21600)}</b></span><span>{watchMetadata?.watch_type==="CRYPTO_TOKEN"?"Token":"Signals"}<b>{watchMetadata?.watch_type==="CRYPTO_TOKEN"?(watchMetadata?.token_symbol||"â€”"):"Project activity"}</b></span></div>}
    ...
    {record.state==="CHALLENGE"?<div className="action-stack">{isOwner&&<button disabled={!client||busy===`presence-${record.record_id}`} className={`secondary-action ${busy===`presence-${record.record_id}`?"is-busy":""}`} onClick={()=>stateAction("presence",record.record_id)}>{busy===`presence-${record.record_id}`?<><span className="btn-spinner"/>{txStage==="consensus"?"Awaiting Consensus":txStage==="submitted"?"Transaction Submitted":txStage==="wallet"?"Confirm in Wallet":"Submittingâ€¦"}</>:"I'm still here â€” cancel recovery"}</button>}<button className={`danger-action ${busy===`finalize-${record.record_id}`?"is-busy":""}`} disabled={!client||secondsLeft>0||busy===`finalize-${record.record_id}`} onClick={()=>stateAction("finalize",record.record_id)}>{busy===`finalize-${record.record_id}`?<><span className="btn-spinner"/>{txStage==="consensus"?"Awaiting Consensus":txStage==="submitted"?"Transaction Submitted":txStage==="wallet"?"Confirm in Wallet":"Submittingâ€¦"}</>:secondsLeft>0?`Finalize in ${secondsLeft}s`:"Finalize recovery"}</button></div>:record.state!=="RECOVERED"&&<button className={`secondary-action full ${busy===`assess-${record.record_id}`?"is-busy":""}`} disabled={!client||busy===`assess-${record.record_id}`} onClick={()=>onAssess(record.record_id)}>{busy===`assess-${record.record_id}`?<><span className="btn-spinner"/>{txStage==="consensus"?"Awaiting Consensus":txStage==="submitted"?"Transaction Submitted":txStage==="wallet"?"Confirm in Wallet":"Submittingâ€¦"}</>:assessed?"Run new assessment":"Run assessment"}</button>}
    {record.mode==="RECOVER"&&record.state==="RECOVERED"&&<div className="recover-actions">{isBeneficiary&&!access?.found&&<button className={`primary-action full ${busy===`claim-${record.record_id}`?"is-busy":""}`} disabled={!client||busy===`claim-${record.record_id}`} onClick={()=>claimAccess(record.record_id)}>{busy===`claim-${record.record_id}`?<><span className="btn-spinner"/>{txStage==="consensus"?"Awaiting Consensus":txStage==="submitted"?"Transaction Submitted":txStage==="wallet"?"Confirm in Wallet":"Submittingâ€¦"}</>:"Claim beneficiary access"}</button>}{isBeneficiary&&access?.found&&<button className={`primary-action full ${busy===`unlock-${record.record_id}`?"is-busy":""}`} disabled={busy===`unlock-${record.record_id}`} onClick={()=>unlockRecovery(record)}>{busy===`unlock-${record.record_id}`?<><span className="btn-spinner"/>Waiting for signatureâ€¦</>:"Unlock recovery information"}</button>}{isOwner&&!isBeneficiary&&<div className="quiet-note">Recovery is finalized. Only {shortAddress(record.beneficiary)} can claim and decrypt this payload.</div>}{!isOwner&&!isBeneficiary&&<div className="quiet-note">This encrypted payload is restricted to its nominated beneficiary.</div>}</div>}
  </article>;}
...
</button><div className="header-right"><span className="network-pill"><i/>Studionet</span>{account&&<button className={`guardian-pill ${guardianStatus?.connected?"connected":""}`} onClick={()=>{setGuardianModal(true);refreshGuardianStatus(account);}} aria-label="Open Telegram Guardian"><img src={guardianMascot} alt="Safetern Guardian"/><span><b>Guardian</b><small>{guardianStatus?.connected?"Connected":guardianStatus?.pending?"Pairing":"Telegram"}</small></span><i className={guardianStatus?.connected?"guardian-dot connected":"guardian-dot"}/></button>}{account?<div className="wallet-wrap"><button className="wallet" onClick={()=>setWalletMenu(v=>!v)}>{shortAddress(account)} <span className="chev">âŒ„</span></button>{walletMenu&&<div className="wallet-menu"><div className="wallet-menu-head"><small>CONNECTED WALLET</small><b>{shortAddress(account)}</b></div><button onClick={()=>navigator.clipboard?.writeText(account)}>Copy full address</button>{presenceRecordId===0&&<button disabled={loading} onClick={()=>{setWalletMenu(false);refresh({force:true,address:account}).catch(()=>{});}}>Refresh records</button>}<div className="wallet-network"><i/>GenLayer Studionet</div><button className={`disconnect ${busy==="disconnect"?"is-busy":""}`} disabled={busy==="disconnect"} onClick={onDisconnect}>{busy==="disconnect"?<><span className="btn-spinner"/>Disconnectingâ€¦</>:"Disconnect wallet"}</button></div>}</div>:<button className={`wallet ${busy==="connect"?"is-busy":""}`} disabled={busy==="connect"} onClick={onConnect}>{busy==="connect"?<><span className="btn-spinner"/>Opening walletâ€¦</>:"Connect wallet"}</button>}</div></header>
    <main className={`workspace ${tab==="home"?"home-workspace":"feature-workspace"}`}>
      {account&&loading&&["home","protect","watch","recover"].includes(tab)&&<div className="records-loading-banner" role="status" aria-live="polite"><span className="loader"/><b>{tab==="protect"?"Loading your Protect recordsâ€¦":tab==="watch"?"Loading your Watch recordsâ€¦":tab==="recover"?"Loading your Recover recordsâ€¦":"Loading your Safetern recordsâ€¦"}</b></div>}
      {presenceRecordId>0&&<section className="emergency-presence"><div><div className="mini-label">OWNER PRESENCE Â· COVENANT #{presenceRecordId}</div><h3>Cancel this recovery challenge with the owner wallet.</h3><p>{presenceTarget?`${presenceTarget.name || `Covenant #${presenceRecordId}`} Â· ${presenceTarget.state || "Onchain"}`:"Connect the owner wallet. Safetern will read only this covenant, not the full dashboard."}</p>{presenceLookupError&&<small className="lookup-warn">{presenceLookupError}</small>}</div>{!account?<button className={`primary-action ${busy==="connect"?"is-busy":""}`} disabled={busy==="connect"} onClick={onConnect}>{busy==="connect"?<><span className="btn-spinner"/>Opening walletâ€¦</>:"Connect owner wallet"}</button>:presenceTarget&&sameAddress(presenceTarget.owner,account)?<button className={`primary-action ${busy===`presence-${presenceRecordId}`?"is-busy":""}`} disabled={Boolean(busy)||presenceTarget.state!=="CHALLENGE"} onClick={()=>stateAction("presence",presenceRecordId)}>{busy===`presence-${presenceRecordId}`?<><span className="btn-spinner"/>Submittingâ€¦</>:presenceTarget.state==="CHALLENGE"?"I'M STILL HERE":`State: ${presenceTarget.state||"Unavailable"}`}</button>:<button className="secondary-action" disabled={Boolean(busy)} onClick={()=>loadPresenceTarget(account)}>Retry covenant check</button>}</section>}
      {tab==="how"?<section className="demo-page how-page">
        ...
        <div className="demo-thesis">Weâ€™re not just measuring inactivity. <b>GenLayer determines what that inactivity means from real-world evidence.</b></div>
        <div className="architecture-stack">
          <div className="architecture-row"><div className="architecture-label">01 Â· INPUTS</div><div className="architecture-cards"><div><b>Public evidence</b><span>Websites Â· repositories Â· project activity Â· configured sources</span></div><div><b>Market observations</b><span>DEX liquidity Â· token markets Â· exchange signals</span></div><div><b>Protocol timing</b><span>Monitoring cadence Â· challenge windows Â· continuity rules</span></div></div></div>
          <div className="architecture-arrow">â†“</div>
          <div className="architecture-row core"><div className="architecture-label">02 Â· AUTONOMOUS LAYER</div><div className="architecture-cards"><div><b>Safetern Guardian</b><span>Runs 24/7, gathers observations, triggers assessment and permissionless finalization.</span></div><div><b>GenLayer Intelligent Contract</b><span>Validators interpret evidence and reach consensus on the continuity classification.</span></div></div></div>
          <div className="architecture-arrow">â†“</div>
          <div className="architecture-row"><div className="architecture-label">03 Â· CONSENSUS OUTPUT</div><div className="state-strip"><span>HEALTHY</span><span>SILENT</span><span>AT_RISK</span><span>INCONCLUSIVE</span><span>ABANDONED</span></div></div>
        </div>
        <div className="how-feature-grid">
          <button onClick={()=>setTab("protect")}><span>PROTECT</span><h3>Continuity covenants</h3><p>For systems you control. GenLayer assessment can open a challenge; the owner retains a veto while present. After expiry, recovery finalization is permissionless.</p><b>Assessment â†’ Challenge â†’ Owner veto or Recovery â†’</b></button>
          <button onClick={()=>setTab("watch")}><span>WATCH</span><h3>Dependency intelligence</h3><p>Monitor projects and crypto dependencies using public and market evidence. Watch is informational only and can never authorize recovery.</p><b>Observe â†’ Interpret â†’ Report â†’</b></button>
          <button onClick={()=>setTab("recover")}><span>RECOVER</span><h3>Encrypted continuity</h3><p>Recovery information is encrypted for a nominated beneficiary. After RECOVERED, beneficiary authorization unlocks it locally in their browser.</p><b>Encrypt â†’ Consensus â†’ Authorize â†’ Decrypt â†’</b></button>
          <div className="how-guardian-card"><span>TELEGRAM GUARDIAN</span><h3>Human notification without human authority</h3><p>Wallet-authorized Telegram pairing delivers meaningful alerts, owner-presence links and recovery notifications. Guardian automates and informs; it cannot sign owner-only or beneficiary-only actions.</p><b>{guardianHealth?.ok?"ONLINE Â· 24/7":"HEALTH ALERTS"}</b></div>
        </div>
        ...
        <div className="demo-proof"><div><div className="mini-label">LIVE DEPLOYMENT</div><h3>The architecture above is running now.</h3></div><div className="demo-proof-items"><span><small>Intelligent Contract</small><b>{shortAddress(SAFETERN_CONTRACT)}</b></span><span><small>Network</small><b>GenLayer Studionet</b></span><span><small>Guardian</small><b>{guardianHealth?.ok?"Online Â· 24/7":"Unavailable"}</b></span><span><small>Verified paths</small><b>Protect #8 Â· Watch #1 Â· Recover #9</b></span></div><button className="primary-action how-demo-cta" onClick={()=>setTab("demo")}>TRY DEMO</button></div>
      </section>:tab==="demo"?<section className="demo-page judge-demo-page">
        <div className="demo-hero"><div><div className="eyebrow"><span/> INTERACTIVE JUDGE DEMO</div><h1>Try Safetern safely.</h1><p>Start instantly with Safeternâ€™s embedded Studionet demo wallets. No MetaMask, browser extension or personal wallet is required. Demo actions use the real deployed Safetern contract.</p></div><div className={`demo-live ${guardianHealth?.ok?"online":""}`}><i/><span><b>{guardianHealth?.ok?"Guardian online":"Guardian unavailable"}</b><small>Real production Guardian</small></span></div></div>
        ...
        <section className="feature-records"><div className="section-title"><div className="mini-label">{tab.toUpperCase()} RECORDS</div><h2>{tab==="watch"?"Your Watches":tab==="protect"?"Your continuity covenants":"Your recovery covenants"}</h2></div>{!account?<div className="empty-state"><div className="empty-icon">â—‡</div><h3>Connect wallet to view your records</h3><p>Protect, Watch and Recover records load only after a wallet is connected.</p><button className="primary-action" onClick={onConnect}>Connect wallet</button></div>:loading?<div className="empty-state"><span className="loader"/><h3>Reading your Safetern records</h3><p>Loading accepted Studionet state for {shortAddress(account)}â€¦</p></div>:currentRows.length===0?<div className="empty-state"><div className="empty-icon">{tab==="watch"?"â—‰":tab==="protect"?"â—‡":"âŒ"}</div><h3>{tab==="watch"?"No Watches yet":tab==="protect"?"No Protect covenants yet":"No recovery covenants for this wallet"}</h3><p>{tab==="watch"?"Create a Watch using public evidence you choose.":tab==="protect"?"Create a continuity covenant for a system you control.":"Create a Recovery Covenant as an owner, or connect a wallet that has been nominated as beneficiary."}</p><button className="primary-action" onClick={openCreate}>{meta.action}</button></div>:<div className="record-grid">{currentRows.map(renderCard)}</div>}</section>
      </>}
    </main>
...
function WizardModal({title,kicker,step,setStep,steps,onClose,busy,txStage,onFinish,finishLabel,finishBusy,creationResult,onSuccessDone,children}) {
  ...
  const content=<div className="modal-backdrop" role="dialog" aria-modal="true" onMouseDown={(e)=>{if(e.target===e.currentTarget&&!busy&&!success)onClose();}}><div className="modal wizard-modal"><div className="modal-head"><div><div className="mini-label">{success?"COMPLETED":kicker}</div><h2>{success?successTitle:title}</h2></div><button type="button" className="close" onClick={()=>{if(!busy)(success?onSuccessDone():onClose())}}>Ã—</button></div>{success?<div className="completion-page"><div className="completion-mark">âœ“</div><div className="mini-label">ONCHAIN & READY</div><h3>{creationResult.name}</h3><p>{creationResult.appeared?"Safetern has confirmed the new record on GenLayer Studionet.":"The transaction was submitted and GenLayer is still finalizing it."}</p><div className="completion-details"><span><small>Record</small><b>{creationResult.recordId?`#${creationResult.recordId}`:"Finalizing"}</b></span>{creationResult.monitoringInterval&&<span><small>Monitoring</small><b>{formatDuration(creationResult.monitoringInterval)}</b></span>}<span><small>Transaction</small><b>{shortAddress(creationResult.hash)}</b></span></div><div className="completion-actions"><button className="secondary-action" onClick={()=>navigator.clipboard?.writeText(creationResult.hash)}>Copy transaction</button><button className="primary-action" onClick={onSuccessDone}>View {creationResult.kind==="watch"?"Watch":creationResult.kind==="recover"?"Recovery":"Covenant"}</button></div></div>:<><div className="wizard-progress" style={{gridTemplateColumns:`repeat(${steps.length}, minmax(0, 1fr))`}}>{steps.map((label,i)=><div key={`${label}-${i}`} className={i===step?"current":i<step?"done":""}><span>{i<step?"âœ“":i+1}</span><small>{label}</small></div>)}</div><form onSubmit={last?onFinish:(e)=>{e.preventDefault();setStep(v=>Math.min(steps.length-1,v+1));}}><div className="wizard-body">{children}</div><div className="modal-footer wizard-footer"><button type="button" className="text-action" onClick={()=>step===0?onClose():setStep(v=>v-1)}>{step===0?"Cancel":"Back"}</button><div className="wizard-footer-right"><span>{step+1} / {steps.length}</span><button className={`primary-action ${finishBusy?"is-busy":""}`} disabled={Boolean(busy)}>{last?(finishBusy?<><span className="btn-spinner"/>{txStage==="consensus"?"Awaiting Consensus":txStage==="submitted"?"Transaction Submitted":txStage==="wallet"?"Confirm in Wallet":txStage==="submitting"?"Submittingâ€¦":"Preparingâ€¦"}</>:finishLabel):"Next"}</button></div></div></form></>}</div></div>;
