...
function App() {
  ...
  useEffect(()=>{
    ...
    const changed=async(accounts)=>{if(demoActiveRef.current)return;const next=accounts?.[0]||"";setAccount(next);setClient(next?walletClient(next):null);const local=getRecoveryIdentity(next);setIdentity(local);setRecipientCode(local?recipientCodeFromIdentity(local):"");setIdentityRegistered(false);if(next){try{const r=await getRegisteredRecoveryIdentity(next);setIdentityRegistered(Boolean(r?.found&&r?.active));}catch{setIdentityRegistered(false);}}setRecords([]);setPresenceTarget(null);setPresenceLookupError("");setGuardianStatus(null);setGuardianPairUrl("");setLoading(false);setNotice({text:next?"Wallet account changed. Click Refresh records to load this wallet's Safetern state.":"Wallet disconnected."});};
  ...
  async function onConnect(){
    ...
    try {
      ...
      try { const registered=await getRegisteredRecoveryIdentity(session.account); setIdentityRegistered(Boolean(registered?.found&&registered?.active)); }
      catch { setIdentityRegistered(false); }
      setNotice({text:"Wallet connected to GenLayer Studio Dev (61997).",kind:"success"});
  ...
  async function startEmbeddedDemo(role=demoRole) {
    setBusy("demo-session"); setNotice({text:"Preparing disposable Safetern Studio Dev demo wallets…"});
    try {
  ...
  async function registerExistingIdentity(){
    if(!identity||!client||!account)return;
    setBusy("identity-register");
    setNotice({text:"Registering this browser Recovery Identity on GenLayer Studio Dev (61997)…"});
    try{
      const code=recipientCodeFromIdentity(identity);
      const result=await registerRecoveryIdentity(client,code,identity.fingerprint);
      const hash=txHash(result);
      setNotice({text:"Registration submitted. Waiting for GenLayer validator consensus…",hash});
      await waitForReceipt(hash);
      const registered=await waitFor(async()=>{const r=await getRegisteredRecoveryIdentity(account);return r?.found&&r?.active?r:null;});
      setIdentityRegistered(Boolean(registered));
      setNotice({text:registered?"Recovery Identity registered on Studio Dev (61997) and ready.":"Registration is still finalizing.",hash,kind:registered?"success":""});
    }catch(e){setNotice({text:actionErrorMessage(e,"Could not register Recovery Identity on Studio Dev."),kind:"error"});}
    finally{setBusy("");}
  }

  async function createIdentity(){setBusy("identity");setNotice({text:"Creating Recovery Identity locally… One wallet signature will protect the private recovery key."});try{const made=await generateRecoveryIdentity(account);const code=recipientCodeFromIdentity(made);setIdentity(made);setRecipientCode(code);setNotice({text:"Recovery Identity created. Confirm one GenLayer transaction to register the public key to this wallet."});const result=await registerRecoveryIdentity(client,code,made.fingerprint);const hash=txHash(result);setNotice({text:"Registration submitted. Waiting for GenLayer validator consensus…",hash});await waitForReceipt(hash);const registered=await waitFor(async()=>{const r=await getRegisteredRecoveryIdentity(account);return r?.found?r:null;});setIdentityRegistered(Boolean(registered?.found&&registered?.active));setNotice({text:registered?"Recovery Identity registered and ready.":"Identity created locally; onchain registration is still finalizing.",hash,kind:registered?"success":""});}catch(e){setNotice({text:actionErrorMessage(e,"Could not create Recovery Identity."),kind:"error"});}finally{setBusy("");}}
  ...
  {demoSession?.token ? "DEMO MODE" : "TRY DEMO"}
    ...
    <footer><span>Safetern v0.3 · Experimental software · Do not use for production secrets</span><span>Contract {shortAddress(SAFETERN_CONTRACT)} · GenLayer Studio Dev (61997)</span></footer>
  </div>;
}
