...
function App() {
  ...
  async function onConnect(){
    ...
    try {
      ...
      setNotice({text:"Wallet connected to GenLayer Studio Dev.",kind:"success"});
  ...
  {demoSession?.token ? "DEMO MODE" : "TRY DEMO"}
    ...
    <main className={`workspace ${tab==="home"?"home-workspace":"feature-workspace"}`}>
      ...
      {tab==="how"?<section className="demo-page how-page">
        ...
        <div className="demo-proof"><div><div className="mini-label">LIVE DEPLOYMENT</div><h3>The architecture above is running now.</h3></div><div className="demo-proof-items"><span><small>Intelligent Contract</small><b>{shortAddress(SAFETERN_CONTRACT)}</b></span><span><small>Network</small><b>GenLayer Studio Dev</b></span><span><small>Guardian</small><b>{guardianHealth?.ok?"Online · 24/7":"Unavailable"}</b></span><span><small>Verified paths</small><b>Protect #8 · Watch #1 · Recover #9</b></span></div><button className="primary-action how-demo-cta" onClick={()=>setTab("demo")}>TRY DEMO</button></div>
        ...
        <div className="demo-proof"><div><div className="mini-label">LIVE SYSTEM</div><h3>Real infrastructure behind the demo.</h3></div><div className="demo-proof-items"><span><small>Contract</small><b>{shortAddress(SAFETERN_CONTRACT)}</b></span><span><small>Network</small><b>GenLayer Studio Dev</b></span><span><small>Guardian</small><b>{guardianHealth?.ok?"Online · 24/7":"Unavailable"}</b></span><span><small>Demo wallet</small><b>{demoSession?.token?shortAddress(account):"Start demo"}</b></span></div></div>
        <div className="demo-next"><div><div className="mini-label">REAL TESTING</div><h3>Want to execute the protocol?</h3><p>Start the embedded demo, then open Protect, Watch or Recover. Transactions are submitted to the real GenLayer Studio Dev contract. Telegram Guardian can be paired directly from the demo wallet.</p></div><div><button className="secondary-action" onClick={()=>setTab("how")}>HOW IT WORKS</button><button className="primary-action" onClick={()=>setTab(demoRole==="beneficiary"?"recover":"protect")}>{demoRole==="beneficiary"?"OPEN RECOVER":"OPEN PROTECT"}</button></div></div>
      </section>:tab==="home"?<>
    ...
    <footer><span>Safetern v0.3 · Experimental software · Do not use for production secrets</span><span>Contract {shortAddress(SAFETERN_CONTRACT)} · GenLayer Studio Dev</span></footer>
  </div>;
}
